package com.farhan.searchengine.services;

import com.farhan.searchengine.config.WikiPageParsingConstants;
import com.farhan.searchengine.model.MergeLine;
import com.farhan.searchengine.model.WikiPage;
...
public class PageParser {
    ...
    /*
     * private int maxFreq(Map<String,Integer[]> freq){ Iterator<Map.Entry<String,
     * Integer[]>> it=freq.entrySet().iterator(); int max=0; int weight=0;
     * while(it.hasNext()){ Map.Entry<String,Integer[]> en=it.next(); weight =
     * getWeight(en.getValue()); if(weight > max) max=weight; } return max; }
     */
    ...
    public void insertToAllWords(WikiPage page, Map<String, Integer[]> wordCount)
        ...
        while (entries.hasNext()) {
            ...
            if (docList == null) {
                ...
                docList.append(page.getId()).append(WikiPageParsingConstants.DOC_COUNT_DELIMITER)
                        .append(getFieldsString(entry.getValue()).append(WikiPageParsingConstants.CHAR_DOC_DELIMITER));
        ...
        if (WikiPageParsingConstants.NUM_OF_PAGES_PER_CHUNK == ++WikiPageParsingConstants.NumOfPagesInMap) {
            System.out.println("dump number " + ++WikiPageParsingConstants.dump + " last dump dur:(s) "
                    + TimeUnit.SECONDS.convert(System.currentTimeMillis() - WikiPageParsingConstants.lastDump,
                            TimeUnit.MILLISECONDS)
                    + " total time(m) " + TimeUnit.MINUTES.convert(
                            System.currentTimeMillis() - WikiPageParsingConstants.startTime, TimeUnit.MILLISECONDS));
    ...
    public void dumpAllWords() throws IOException {
        ...
        while (entries.hasNext()) {
            ...
            entry = entries.next();
            blockOfData.append(entry.getKey()).append(WikiPageParsingConstants.WORD_DELIMITER).append(entry.getValue())
                    .append("\n");
    ...
    public void writeData(String blockOfData, Writer writer) {
        try {
            if (WikiPageParsingConstants.prevWriter != writer) {
                ...
                System.out.println("file number " + ++WikiPageParsingConstants.indexCount + " last file dur:(s) "
                        + TimeUnit.SECONDS.convert(System.currentTimeMillis() - WikiPageParsingConstants.lastDump,
                                TimeUnit.MILLISECONDS)
                        + " total time(m) "
                        + TimeUnit.MINUTES.convert(System.currentTimeMillis() - WikiPageParsingConstants.startTime,
                                TimeUnit.MILLISECONDS));
                WikiPageParsingConstants.lastDump = System.currentTimeMillis();
            }
    ...
    /*
     * public void printPostingList(){ Iterator< Map.Entry<String, StringBuilder> >
     * entries=allWords.entrySet().iterator(); PrintWriter writer=null; File
     * indexFile=null; try { indexFile =new
     * File(WikiPageParsingConstants.indexFileDir, "posting_list.txt") ;
     * 
     * 
     * writer=new PrintWriter(new FileOutputStream(indexFile));
     * 
     * 
     * while(entries.hasNext()){ Map.Entry<String,StringBuilder>
     * entry=entries.next();
     * 
     * writer.println(entry.getKey() + WikiPageParsingConstants.WORD_DELIMITER +
     * entry.getValue());
     * 
     * }
     * 
     * } catch (FileNotFoundException e) { e.printStackTrace(); } finally{ if(writer
     * != null) writer.close(); }
     * ExternalSort.createOffsetsFile(indexFile.getAbsolutePath(),
     * WikiPageParsingConstants.OFFSETS_FILE,100000); }
     */
    ...
    public void mergeSubIndexFiles() throws IOException {
        ...
        PriorityQueue<MergeLine> pq = new PriorityQueue<MergeLine>(WikiPageParsingConstants.subIndexFiles.size());
        ...
        for (int i = 0; i < WikiPageParsingConstants.NUM_OF_INDEXFILES; i++) {
            auxIndexFile = new File(WikiPageParsingConstants.indexFileDir, i + WikiPageParsingConstants.INDEX_SUFFIX);
        ...
        while (!pq.isEmpty()) {
            ...
            if (currentWord.equals(mergeLine.getWord())) {
                ...
                allWordsWriter
                        .write(currentWord + WikiPageParsingConstants.WORD_IDF_DELIMITER + numOfDocsWordPresent + "\n");
                idf = invertedDocumentFreq(numOfDocsWordPresent);
                StringBuffer wholeLine = new StringBuffer();
                // Add word
                wholeLine.append(currentWord).append(WikiPageParsingConstants.WORD_IDF_DELIMITER).append(idf)
    ...
    /*
     * public void printLineToIndexFile(List<MergeLine> sameWords, String
     * currentWord, BufferedWriter indexFileWriter,MergeLine mergeLine) {
     * StringBuffer wholeLine=new StringBuffer(); //Add word
     * wholeLine.append(currentWord).append(WikiPageParsingConstants.WORD_DELIMITER)
     * ; //append docIds for(MergeLine sameWord:sameWords){
     * wholeLine.append(sameWord.getDocIds()); }
     * 
     * wholeLine.append("\n");
     * 
     * writeData(wholeLine.toString(), indexFileWriter); sameWords=new
     * ArrayList<MergeLine>(); sameWords.add(mergeLine);
     * currentWord=mergeLine.getWord();
     * 
     * }
     */
    ...
    public void nextMergeLine(PriorityQueue<MergeLine> pq, List<BufferedReader> readers, int readerNum,
            boolean[] reachedEOF) throws IOException {
    ...
    /*
     * private int countChar(String s,char ch){ int times=0; for(int
     * i=0;i<s.length();i++){ if(ch == s.charAt(i)) times++; } return times; }
     */
