...
public class ExternalSort {
	...
	public static Map<String, Long> getOffsets(String offSetFileDir) {
		...
		int Count = 0;
		try {
			address = new BufferedReader(
					new FileReader(new File(offSetFileDir, WikiPageParsingConstants.OFFSETS_FILE)));
		...
		/*
		 * RandomAccessFile index=new RandomAccessFile(new File("index.txt"), "r"); int
		 * lines=0; for( Long offset: offsets.values()){ index.seek(offset);
	...
	public static void createOffsetsFile(String indexFile, String offsetsFileDir, int offsetFilePrefix) {
		...
		try {
			...
			// "+WikiPageParsingConstants.INDEX_FILE_NAME);
